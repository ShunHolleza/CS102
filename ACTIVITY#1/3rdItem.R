...
URL <- "https://www.amazon.com/s?k=basketball+shoes+for+men+nike&crid=3N1HEAQVRVNE7&sprefix=basketball+shoes+for+men+%2Caps%2C482&ref=nb_sb_ss_ts-doa-p_1_25"
...
write.csv(DF, "3rdproduct.csv")

DF$Category <- "BasketballShoes"

DF <- DF[, c("Category", names(DF)[-which(names(DF) == "Category")])]

write.csv(DF, "3rdproduct2.csv", row.names = FALSE)
