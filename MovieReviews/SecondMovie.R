...
url2 <- 'https://www.imdb.com/title/tt4154756/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrapePage <- function(url2, PaginationKey) {
  session <- bow(sprintf(url2, PaginationKey), user_agent = "Educational")
...
for (page in 1:pagesToscrape) {
  scrapedData <- scrapePage(url2, PaginationKey)
...
DF <- data.frame(
  Movie_Title = rep("Avengers: Infinity War", reviewsToscrape),
