...
url9<- 'https://www.imdb.com/title/tt0948470/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrapePage <- function(url9, PaginationKey) {
...
for (page in 1:pagesToscrape) {
  scrapedData <- scrapePage(url9, PaginationKey)
...
write.csv(DF, file = "9thmovie.csv", row.names = FALSE)
print(DF)
