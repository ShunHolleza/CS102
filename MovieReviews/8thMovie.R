...
url8 <- 'https://www.imdb.com/title/tt0145487/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrapePage <- function(url8, PaginationKey) {
...
for (page in 1:pagesToscrape) {
  scrapedData <- scrapePage(url8, PaginationKey)
...
DF <- data.frame(
  Movie_Title = rep("Spider-Man", reviewsToscrape),
...
write.csv(DF, file = "8thmovie.csv", row.names = FALSE)
print(DF)
