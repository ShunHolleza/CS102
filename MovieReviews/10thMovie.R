...
url10 <- 'https://www.imdb.com/title/tt4633694/reviews/_ajax?ref_=undefined&paginationKey=%s'


scrapePage <- function(url10, PaginationKey) {
  session <- bow(sprintf(url10, PaginationKey), user_agent = "Educational")
...
for (page in 1:pagesToscrape) {
  scrapedData <- scrapePage(url10, PaginationKey)
...
DF <- data.frame(
  Movie_Title = rep("Spider-Man: Into the Spider-Verse", reviewsToscrape),
...
write.csv(DF, file = "10thmovie.csv", row.names = FALSE)
print(DF)
